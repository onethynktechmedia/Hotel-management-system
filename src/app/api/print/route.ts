...
export async function POST(request: NextRequest) {
  try {
    ...
    const { billContent, printerName = 'EC58' } = body
    ...
    if (process.env.VERCEL || process.env.AWS_LAMBDA_FUNCTION_NAME) {
      console.log('Running in serverless environment, using WebUSB')
      return NextResponse.json({ 
        success: true, 
        message: 'Use WebUSB for printing in production.',
        useWebUSB: true,
        billContent: billContent
      }, { status: 200 })
    }

    // For local development with CUPS
    try {
